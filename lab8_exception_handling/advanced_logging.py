...
if __name__ == "__main__":
    ...
    data_list = [
        {"id": 1, "name": "Valid Data"},
        {"no_id": True, "name": "Missing ID"},
    ]
    
    files_to_test = []
    
    # 1. Create a valid file
    with open("log_test_valid.json", "w") as f:
        json.dump(data_list[0], f)
    files_to_test.append("log_test_valid.json")
    
    # 2. Create a file missing ID (triggers warning)
    with open("log_test_no_id.json", "w") as f:
        json.dump(data_list[1], f)
    files_to_test.append("log_test_no_id.json")
    
    # 3. Create a malformed file (triggers error)
    with open("log_test_malformed.json", "w") as f:
        f.write("{ invalid json ")
    files_to_test.append("log_test_malformed.json")
    
    # 4. Add a non-existent file
    files_to_test.append("non_existent_file.json")

    print("Running DataProcessor with Advanced Logging...")
    processor = DataProcessor()
    processor.batch_process(files_to_test)
    
    stats = processor.get_statistics()
    print(f"\nFinal Statistics: {stats}")
    print("\nCheck the 'logs/' directory for debug.log, errors.log, and app_rotating.log")
